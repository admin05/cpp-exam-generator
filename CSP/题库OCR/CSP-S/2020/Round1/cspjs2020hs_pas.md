...
A. b                      B. a                      C. d                      D. c
5. 将 (2, 7, 10, 18) 分别存储到某个地址区间为 0~10 的哈希表中，如果哈希函数  $h(x) = (\text{☐})$ ，将不会产生冲突，其中  $a \bmod b$  表示  $a$  除以  $b$  的余数。  
A.  $x^2 \bmod 11$   
B.  $2x \bmod 11$   
C.  $x \bmod 11$   
...
- 3) (2.5 分) 当输入的  $d[i]$  是严格**单调递增**序列时, 第 27 行的 “swap” 平均执行次数是 ( )。  
A.  $\theta(n \log n)$     B.  $\theta(n)$     C.  $\theta(\log n)$     D.  $\theta(n^2)$
- 4) (2.5 分) 当输入的  $d[i]$  是严格**单调递减**序列时, 第 27 行的 “swap” 平均执行次数是 ( )。  
...
![CCF logo: A circular emblem with '中国计算机学会' (Chinese Computer Society) at the top, 'CCF' at the bottom, and a stylized globe with arrows in the center.](440e59dae4772c0152116a3abd34331a_img.jpg)

CCF logo: A circular emblem with '中国计算机学会' (Chinese Computer Society) at the top, 'CCF' at the bottom, and a stylized globe with arrows in the center.
...
![CCF logo: A circular emblem with '中国计算机学会' (Chinese Computer Society) at the top, 'CCF' at the bottom, and a stylized globe with arrows in the center.](a68671ef36320b8515aa85d288d82cf1_img.jpg)
...
![CCF logo: A circular emblem with '中国计算机学会' (Chinese Computer Society) at the top, 'CCF' at the bottom, and a stylized globe with arrows in the center.](c834b9abb4ddf70e5d10641f87d5ff5b_img.jpg)
...
![CCF logo: 中国计算机学会 CCF](967c30813761a8952ecc5e16bf42ea45_img.jpg)
...
![CCF Logo](d5a837fa4f4675e5ee596003cf55985c_img.jpg)
...
2. (最优子序列) 取 m = 16, 给出长度为n的整数序列a1, a2, ..., an(0 ≤ ai < 2^m)。对于一个二进制数x, 定义其分值w(x)为x + popcnt(x), 其中 popcnt(x)表示 x 二进制表示中 1 的个数。对于一个子序列b1, b2, ..., bk, 定义其子序列分值S为w(b1 ⊕ b2) + w(b2 ⊕ b3) + w(b3 ⊕ b4) + ... + w(bk-1 ⊕ bk)。其中⊕表示按位异或。对于空子序列, 规定其子序列分值为 0。求一个子序列使得其子序列分值最大, 输出这个最大值。

输入第一行包含一个整数 n(1 ≤ n ≤ 40000)。接下来一行包含n个整数 a1, a2, ..., an。
...
![CCF logo: 中国计算机学会 CCF](0f2a1e4a7b12fe5b8749882ecd636f5c_img.jpg)
...
![CCF logo: 中国计算机学会 CCF](0dd5ee731e9d7e34e498b5c926110773_img.jpg)
...
- 2) ②处应填 ( )  
A.  $(a \text{ and MS}) \text{ shl } B$                       B.  $a \text{ shr } B$   
C.  $a \text{ and } (1 \text{ shl } B)$                       D.  $a \text{ and } (\text{MS} \text{ shl } B)$
- 3) ③处应填 ( )  
A.  $-\text{INF}$                       B.  $\text{Max}[y][x]$   
C.  $0$                       D.  $\text{Max}[x][y]$
- 4) ④处应填 ( )  
