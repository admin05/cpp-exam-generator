...
- 3) (2.5 分) 当输入的  $d[i]$  是严格**单调递增**序列时, 第 19 行的 “swap” 平均执行次数是 ( )。  
A.  $\theta(n \log n)$     B.  $\theta(n)$     C.  $\theta(\log n)$     D.  $\theta(n^2)$
- 4) (2.5 分) 当输入的  $d[i]$  是严格**单调递减**序列时, 第 19 行的 “swap” 平均执行次数是 ( )。  
...
![CCF logo: A circular emblem with '中国计算机学会' (Chinese Computer Society) at the top and 'CCF' at the bottom, surrounding a stylized globe with arrows.](440e59dae4772c0152116a3abd34331a_img.jpg)

CCF logo: A circular emblem with '中国计算机学会' (Chinese Computer Society) at the top and 'CCF' at the bottom, surrounding a stylized globe with arrows.
...
![CCF logo: A circular emblem with '中国计算机学会' at the top, 'CCF' at the bottom, and a stylized globe in the center.](a68671ef36320b8515aa85d288d82cf1_img.jpg)

CCF logo: A circular emblem with '中国计算机学会' at the top, 'CCF' at the bottom, and a stylized globe in the center.
...
![CCF logo](967c30813761a8952ecc5e16bf42ea45_img.jpg)
...
![CCF logo: 中国计算机学会 CCF](d5a837fa4f4675e5ee596003cf55985c_img.jpg)
...
C.  $0$                       D.  $Max[x][y]$
- 4) ④处应填 ( )
- A.  $Max[x][z] + w(y \hat{=} z)$                       B.  $Max[x][z] + w(a \hat{=} z)$   
C.  $Max[x][z] + w(x \hat{=} (z \ \ll \ B))$                       D.  $Max[x][z] + w(x \hat{=} z)$
- 5) ⑤处应填 ( )
- A.  $to\_max(\&Max[y][z], v + w(a \hat{=} (z \ \ll \ B)))$   
B.  $to\_max(\&Max[z][y], v + w((x \hat{=} z) \ \ll \ B))$   
C.  $to\_max(\&Max[z][y], v + w(a \hat{=} (z \ \ll \ B)))$   
D.  $to\_max(\&Max[x][z], v + w(y \hat{=} z))$