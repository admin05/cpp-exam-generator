...
- Image: A diagram of a graph with 4 vertices and 5 edges. The vertices are arranged in a roughly rectangular shape. There is an edge between the top-left and top-right vertices, an edge between the top-right and bottom-right vertices, an edge between the bottom-right and bottom-left vertices, and an edge between the bottom-left and top-left vertices, forming a cycle. Additionally, there is a diagonal edge connecting the top-left and bottom-right vertices.
- A. 所有顶点的度数均为偶数  
B. 该图连通  
C. 该图存在一个欧拉回路  
D. 该图的边数是奇数
...
- C. 630
- D. 5040
- Image: Hand-drawn diagrams illustrating a 4-cycle (a square) and a 5-cycle (a pentagon) in a complete graph.

The diagram shows two hand-drawn graphs. On the left is a square with four vertices and four edges, representing a cycle of length 4. On the right is a pentagon with five vertices and five edges, representing a cycle of length 5. These diagrams are likely intended to illustrate the concept of cycles in a complete graph.
...
![A directed graph with 7 nodes (1-7). Node 1 has edges to 2, 3, and 4. Node 2 has edges to 4 and 5. Node 3 has an edge to 4. Node 4 has edges to 5 and 6. Node 5 has edges to 6 and 7. Node 6 has an edge to 7.](f0bae10b54c4f3cf8d0e33f5e2fb7cfa_img.jpg)

```
graph LR; 1((1)) --> 2((2)); 1((1)) --> 3((3)); 1((1)) --> 4((4)); 2((2)) --> 4((4)); 2((2)) --> 5((5)); 3((3)) --> 4((4)); 4((4)) --> 5((5)); 4((4)) --> 6((6)); 5((5)) --> 6((6)); 5((5)) --> 7((7)); 6((6)) --> 7((7));
```

A directed graph with 7 nodes (1-7). Node 1 has edges to 2, 3, and 4. Node 2 has edges to 4 and 5. Node 3 has an edge to 4. Node 4 has edges to 5 and 6. Node 5 has edges to 6 and 7. Node 6 has an edge to 7.
...
- 19. 函数 `int logic(int x, int y)` 的功能是 ( )
  ...
  - C. 按位异或
  - D. 以上都不是
- 20. (4分) 当输入为 "10 100 100" 时, 输出的第 100 个数是 ( )
  - A. 91
  - B. 94
  - C. 95
  - D. 98
...
21. 假设数组  $dp$  长度无限制，函数  $\text{solve}()$  所实现的算法的时间复杂度是  $O(n \cdot 2^m)$ 。( )

{7}------------------------------------------------

22. 输入“11 2 10000000001”时，程序输出两个数 32 和 23。（ ）

23. (2分) 在  $n \leq 10$  时，solve() 的返回值始终小于  $4^{10}$ 。（X）
...
- 27. 假设程序运行前能自动将 maxn 改为 n+1，所实现的算法的时间复杂度是  $O(n \log n)$ 。(X)
- 28. 时间开销的瓶颈是 init() 函数。( )
...
- A. a[mid] > ai B. a[mid] >= ai
C. a[mid] < ai D. a[mid] <= ai
...
- A. get\_rank(mid) < k B. get\_rank(mid) <= k
C. get\_rank(mid) > k D. get\_rank(mid) >= k
...
- A. upd(pre[b], n+b, dis[b], q)    B. upd(a, n+b, d, q)  
C. upd(pre[b], b, dis[b], q)    D. upd(a, b, d, q)

39.②处应填 ( )

- A. make\_pair(-d, b)                      B. make\_pair(d, b)  
C. make\_pair(b, d)                      D. make\_pair(-b, d)
...
- A. upd(a, n+b, dis[a]+c, q)    B. upd(n+a, n+b, dis2[a]+c, q)  
C. upd(n+a, b, dis2[a]+c, q)    D. upd(a, b, dis[a]+c, q)
