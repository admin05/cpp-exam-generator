...
15. 有正实数构成的数字三角形排列形式如图所示。第一行的数为 a1,1；第二行的数从左到右依次为 a2,1, a2,2，第 n 行的数为 an,1, an,2, ..., an,n。从 a1,1 开始，每一行的数 ai,j 只有两条边可以分别通向下一行的两个数 ai+1,j 和 ai+1,j+1。用动态规划算法找出一条从 a1,1 向下通到 an,1, an,2, ..., an,n 中某个数的路径，使得该路径上的数之和达到最大。

![Diagram of a number triangle showing elements a1,1, a2,1, a2,2, a3,1, a3,2, a3,3 and so on.](6343a896ff7c6a10ec2f48bad236b8bf_img.jpg)

Diagram of a number triangle showing elements a1,1, a2,1, a2,2, a3,1, a3,2, a3,3 and so on.
...
4 #define maxl 202
5 char s[maxl], t[maxl];
6 int pre[maxl], suf[maxl];
7
8 int main() {
9     scanf("%s%s", s, t);
10    int slen = strlen(s), tlen = strlen(t);
...
1 #include <stdio.h>
2 #include <stdbool.h>
3 #define swap(x, y) { int t = x; x = y; y = t; }
4
5 #define maxn 64
6
7 int n, m;
