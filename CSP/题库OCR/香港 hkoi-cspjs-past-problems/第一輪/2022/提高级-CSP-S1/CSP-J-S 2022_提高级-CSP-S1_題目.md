...
C. DE、EF  
D. DE、DE
- 7. 一个深度为 5（根结点深度为 1）的完全 3 叉树，按前序遍历的顺序给结点从 1 开始编号，则第 100 号结点的父结点第（ ）号。
...
11. 小明希望选到形如“省 A · LL DDD”的车牌号。车牌号在“·”之前的内容固定不变；后面的 5 位号码中，前 2 位必须是大写英文字母，后 3 位必须是阿拉伯数字（L 代表 A 至 Z，D 表示 0 至 9，两个 L 和三个 D 之间可能相同也可能不同）。请问总共有多少个可供选择的车牌号。( )
...
- 21. 当输入为 “baaabaabaabaabaaaa aaaa”，第 20 行的 “j++” 语句执行次数为 ( )。
- A. 9      B. 10      C. 11      D. 12
...
30. 除非输入的  $n$  为 0，否则程序输出的字符数为  $O(\lfloor \log_k |n| \rfloor + 1)$ 。 ( )
