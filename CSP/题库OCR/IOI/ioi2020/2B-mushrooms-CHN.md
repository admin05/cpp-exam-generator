...
幸运的是，安德鲁的实验室里有一台机器可以帮助他。在使用这台机器时，需要将两个或者多个蘑菇放到机器里，并摆成一排（以任意顺序），然后打开机器。接下来，这台机器会计算所有不属于同一种类的相邻蘑菇对的个数。例如，如果你把种类为 [A, B, B, A] 的蘑菇（按照这个顺序）放到机器中，结果应该是 2。

但是，因为机器操作非常昂贵，机器只能使用有限的次数。此外，在机器的所有使用中，放置到机器中的蘑菇总数不能超过 100 000。请使用这台机器帮助安德鲁来数一数他采集了多少个种类为A的蘑菇。
...
• 假设数组 x 的长度为 d。那么， 此函数返回不同的下标 j 的个数，满足 0 ≤ j ≤ d - 2 并且 x[j] 和 x[j + 1] 属于不同种类。
• 该函数最多可以被调用 20 000 次。
• 在对函数 use\_machine 的所有调用中，所有被传到该函数 use\_machine 的 x 的总长度不能超过 100 000。
...
评测程序示例读入一个整数数组  $s$ ，该数组给出了蘑菇的种类。对于所有  $0 \leq i \leq n - 1$ ， $s[i] = 0$  表示蘑菇  $i$  的种类是A， $s[i] = 1$  表示蘑菇  $i$  的种类是B。评测程序示例读取如下格式的输入数据：
