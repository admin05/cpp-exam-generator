...
对任意两座山峰  $i$  和  $j$  (其中  $0 \leq i < j < N$ )，它们的**距离**定义为  $d(i, j) = j - i$ 。根据古老的印加传说，三座山峰是**神话**三峰的条件是：它们的高度与两两之间的距离在**忽略顺序**后**匹配**。
...
```
long long count_triplets(std::vector<int> H)
```
...
- $3 \leq N \leq 200\,000$
- 对每个满足  $0 \leq i < N$  的  $i$ ，都有  $1 \leq H[i] \leq N - 1$
...
| 3   | 10 | $N \leq 2000$                                                            |
| 4   | 11 | 山峰的高度是单调不下降的。<br>也就是说, 对每个满足 $1 \leq i < N$ 的 $i$ 都有 $H[i-1] \leq H[i]$。 |
| 5   | 16 | $N \leq 50\,000$                                                         |
| 6   | 19 | 没有额外的约束条件。                                                               |
