...
![A 7x7 grid representing a maze. The grid has row indices -1 to 4 and column indices -1 to 5. The outermost cells (rows -1 and 4, columns -1 and 5) are shaded dark gray, representing the boundary. The inner 5x5 area (rows 0 to 3, columns 0 to 4) contains mostly white cells with the number '0' in them, representing empty cells. The cell at (1, 3) is shaded light gray and has a red 'X' through it, representing an obstacle. The columns are labeled -1, 0, 1, 2, 3, 4, 5 at the top, and the rows are labeled -1, 0, 1, 2, 3, 4 on the left.](59c76d920f3faf3933d80a47647d42a9_img.jpg)

A 7x7 grid representing a maze. The grid has row indices -1 to 4 and column indices -1 to 5. The outermost cells (rows -1 and 4, columns -1 and 5) are shaded dark gray, representing the boundary. The inner 5x5 area (rows 0 to 3, columns 0 to 4) contains mostly white cells with the number '0' in them, representing empty cells. The cell at (1, 3) is shaded light gray and has a red 'X' through it, representing an obstacle. The columns are labeled -1, 0, 1, 2, 3, 4, 5 at the top, and the rows are labeled -1, 0, 1, 2, 3, 4 on the left.
...
例如，考虑下图左侧显示的场景。Pulibot 当前位于单元格  $(0, 0)$ ，颜色为 0。Pulibot 识别出状态数组  $S = [0, -2, 2, 2, -2]$ 。Pulibot 可能有一个程序，该程序根据所识别的数组，将当前单元格的顏色设置为  $Z = 1$ ，然后向东移动，如图的中间和右侧所示：

{2}------------------------------------------------

![Three 6x6 grid diagrams showing the robot's path. The first grid shows the robot at (0,0) with a path of 2s and 4s. The second grid shows the robot at (0,1) with a path of 2s and 4s. The third grid shows the robot at (0,2) with a path of 1s and 4s. The path is marked with red 'X' marks on the grid cells.](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)

Three 6x6 grid diagrams showing the robot's path. The first grid shows the robot at (0,0) with a path of 2s and 4s. The second grid shows the robot at (0,1) with a path of 2s and 4s. The third grid shows the robot at (0,2) with a path of 1s and 4s. The path is marked with red 'X' marks on the grid cells.
...
![Two 6x6 grid diagrams. The left grid shows the initial configuration with the robot at (0,0) and a path of 2s and 4s. The right grid shows the final configuration with the robot at (0,2) and a path of 1s and 4s. The path is marked with red 'X' marks on the grid cells.](163688ca8da9787f5d48edd68d8cc75b_img.jpg)

Two 6x6 grid diagrams. The left grid shows the initial configuration with the robot at (0,0) and a path of 2s and 4s. The right grid shows the final configuration with the robot at (0,2) and a path of 1s and 4s. The path is marked with red 'X' marks on the grid cells.
...
不需要对每个可能的状态数组  $S$  调用 set\_instruction。但是, 如果 Pulibot 后来识别出未设置指令的状态数组, 你将得到 Output isn't correct 的判定结果。

program\_pulibot 完成后, 评测程序会在一个或多个迷宫上调用 Pulibot 的程序。这些调用**不计入**解决方案的时间限制。评测程序**不是**自适应的, 也就是说, 每个测试用例的迷宫集合都是预先确定的。

如果 Pulibot 在终止程序之前违反了任何机器人比赛规则, 你将得到 Output isn't correct 的判定结果。

### 例子

函数 program\_pulibot 可以调用 set\_instruction 如下:
...
![A 5x5 grid representing a maze. The grid has coordinates from -1 to 3 on both axes. The robot is at (0,0). Cells (0,1), (1,1), and (2,1) are marked with red 'X's, indicating obstacles. Cells (0,0), (1,0), and (2,0) contain the number '0'.](de6e8b740c69dac308cce9edfec3eff4_img.jpg)

A 5x5 grid representing a maze. The grid has coordinates from -1 to 3 on both axes. The robot is at (0,0). Cells (0,1), (1,1), and (2,1) are marked with red 'X's, indicating obstacles. Cells (0,0), (1,0), and (2,0) contain the number '0'.
...
![Five 5x5 grid diagrams showing the robot's path. The robot starts at (0,0). In the first step, it moves to (1,0). In the second step, it moves to (2,0). In the third step, it moves to (3,0). In the fourth step, it moves to (3,1). The final step shows the robot at (3,1) with the path cells (0,0), (1,0), (2,0), (3,0), and (3,1) colored blue and labeled '1'.](17a042ee648d9fdaddb609aead503980_img.jpg)

Five 5x5 grid diagrams showing the robot's path. The robot starts at (0,0). In the first step, it moves to (1,0). In the second step, it moves to (2,0). In the third step, it moves to (3,0). In the fourth step, it moves to (3,1). The final step shows the robot at (3,1) with the path cells (0,0), (1,0), (2,0), (3,0), and (3,1) colored blue and labeled '1'.

但是，注意这个由 4 条指令构成的程序有可能在其他合法的迷宫中找不到最短路径。所以，如果这个程序被提交，它会收到 Output isn't correct 的判定结果。

### 约束条件

$Z_{MAX} = 19$ 。因此，Pulibot 可以使用 0 到 19 的颜色，包含 0 和 19。
