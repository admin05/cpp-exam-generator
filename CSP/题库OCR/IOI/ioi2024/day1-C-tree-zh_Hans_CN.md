...
![A tree diagram with 6 nodes (0-5) and directed edges pointing to parents: 1 to 0, 2 to 0, 3 to 2, 4 to 2, 5 to 2.](c2ddad56c1553f2499aaf7ef8cef6777_img.jpg)

A tree diagram with 6 nodes (0-5) and directed edges pointing to parents: 1 to 0, 2 to 0, 3 to 2, 4 to 2, 5 to 2.
...
对于一个给定的系数序列 C[0], ..., C[N - 1]，结点 i 的代价为 |C[i]| · W[i]，这里 |C[i]| 表示 C[i] 的绝对值。最后，总体代价为所有结点的代价之和。你的任务是，对于每个询问，计算出可以由某个有效系数
...
![A tree diagram with 3 nodes. Node 0 is the root, and nodes 1 and 2 are its children. Each node has a weight of 1. Node 0 has a coefficient of -1 (shown in a shaded rectangle). Node 1 has a coefficient of 1 (shown in a shaded rectangle). Node 2 has a coefficient of 1 (shown in a shaded rectangle). Arrows point from nodes 1 and 2 to node 0.](2df09d249c38abee55dbf0526dd6da0a_img.jpg)

```
graph BT; 1((1)) --> 0((0)); 2((2)) --> 0((0));
```

A tree diagram with 3 nodes. Node 0 is the root, and nodes 1 and 2 are its children. Each node has a weight of 1. Node 0 has a coefficient of -1 (shown in a shaded rectangle). Node 1 has a coefficient of 1 (shown in a shaded rectangle). Node 2 has a coefficient of 1 (shown in a shaded rectangle). Arrows point from nodes 1 and 2 to node 0.
