...
对于某一个测试数据, 如果你的输出不是有效的, 你的这个测试数据的得分将会是 0。反之, 你的得分是  $\min(10, 10 \cdot l/k)$ , 向下取值至小数后二位, 这里的  $l$  是指你输出的迷宫中能够最多藏着的小孩数目, 而  $k$  则表示在输入文件中题目要求你躲藏的小孩数目。对于一个测试数据, 你能够得到 10 分, 当且仅当你的输出是一个能够躲藏  $k$  个或更多小孩的迷宫。
