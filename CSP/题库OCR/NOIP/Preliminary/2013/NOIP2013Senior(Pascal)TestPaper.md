...
![A graph with 5 vertices and 8 edges. It consists of a cycle of 5 vertices with all possible 5 edges, plus 3 additional edges connecting the vertices in a star-like pattern from one vertex to the other four.](c2ddad56c1553f2499aaf7ef8cef6777_img.jpg)

A graph with 5 vertices and 8 edges. It consists of a cycle of 5 vertices with all possible 5 edges, plus 3 additional edges connecting the vertices in a star-like pattern from one vertex to the other four.
...
C.  $2x \bmod 11$                       D.  $[\sqrt{x}] \bmod 11$ , 其中  $[\sqrt{x}]$  表示  $\sqrt{x}$  下取整
...
|    |                                                                     |    |                                                                           |
|----|---------------------------------------------------------------------|----|---------------------------------------------------------------------------|
| A. | <pre>for i := 1 to 100 do sum := sum + i;</pre>                     | B. | <pre>i := 1; while i &gt; 100 do begin sum := sum + i; inc(i); end;</pre> |
| C. | <pre>i := 1; repeat sum := sum + i; inc(i); until i &gt; 100;</pre> | D. | <pre>i := 1; repeat sum := sum + i; inc(i); until i &lt;= 100;</pre>      |
...
![An undirected graph with 5 vertices labeled A0, A1, A2, A3, and A4. A0 is at the top, A1 is on the left, A2 is at the bottom, A3 is on the right, and A4 is at the top right. The edges are: (A0, A1), (A0, A3), (A1, A2), (A2, A3), and (A3, A4).](aec7150315249ac202a63cc0e32323b8_img.jpg)

An undirected graph with 5 vertices labeled A0, A1, A2, A3, and A4. A0 is at the top, A1 is on the left, A2 is at the bottom, A3 is on the right, and A4 is at the top right. The edges are: (A0, A1), (A0, A3), (A1, A2), (A2, A3), and (A3, A4).
...
![Diagram showing five numbered circles (1 to 5) connected by horizontal lines, representing a sequence of lily pads.](37a6ab1d23efb9dc00cfae09d353b1da_img.jpg)

```
graph LR; 1((1)) --- 2((2)); 2 --- 3((3)); 3 --- 4((4)); 4 --- 5((5));
```

Diagram showing five numbered circles (1 to 5) connected by horizontal lines, representing a sequence of lily pads.
...
- 2. （两元序列）试求一个整数序列中，最长的仅包含两个不同整数的连续子序列。如有多个子序列并列最长，输出任意一个即可。例如，序列“1 1 2 3 2 3 2 3 3 1 1 1 3 1”中，有两段满足条件的最长子序列，长度均为 7，分别用下划线和上划线标出。
