...
（ ），将不会产生冲突，其中  $a \bmod b$  表示  $a$  除以  $b$  的余数。  
...
The diagram shows an undirected graph with 4 vertices labeled  $A_0, A_1, A_2, A_3$ . The edges are as follows:  $A_0$  is connected to  $A_1$ ,  $A_2$ , and  $A_3$ ;  $A_1$  is connected to  $A_2$ ; and  $A_2$  is connected to  $A_3$ . This forms a triangle  $A_0-A_1-A_2-A_3$  with an additional edge between  $A_1$  and  $A_2$ .
