...
5. 在 C 语言中，表达式  $21^2$  的值是（ ）

- A. 441      B. 42      C. 23      D. 24      E. 25

6. 在 C 语言中，判断 a 不等于 0 且 b 不等于 0 的正确的条件表达式是（ ）
...
11. 设 A=B=D=true, C=E=false, 以下逻辑运算表达式值为真的有（ ）。
...
13. 设栈 S 的初始状态为空，元素 a, b, c, d, e 依次入栈，以下出栈序列不可能出现的有（ ）。
...
![A triangular grid diagram with a path of dots and lines starting from the top vertex and ending at a dot near the bottom right. The path consists of 10 segments connecting 11 dots. The grid is composed of 16 small equilateral triangles arranged in a larger equilateral triangle shape.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

A triangular grid diagram with a path of dots and lines starting from the top vertex and ending at a dot near the bottom right. The path consists of 10 segments connecting 11 dots. The grid is composed of 16 small equilateral triangles arranged in a larger equilateral triangle shape.
