...
13. 有正实数构成的数字三角形排列形式如图所示。第一行的数为  $a_{11}$ ；第二行的数从左到右依次为  $a_{21}, a_{22}$ ；...第 n 行的数为  $a_{n1}, a_{n2}, \dots, a_{nn}$ 。从  $a_{11}$  开始，每一行的数  $a_{ij}$  只有两条边可以分别通向下一行的两个数  $a_{(i+1)j}$  和  $a_{(i+1)(j+1)}$ 。用动态规划算法找出一条从  $a_{11}$  向下通到  $a_{n1}, a_{n2}, \dots, a_{nn}$  中某

![Diagram of a number triangle with elements a11, a21 a22, a31 a32 a33, ..., an1 an2 ... ann.](a2dc8d6396228142d4c2c1d3160b47dc_img.jpg)

Diagram of a number triangle with elements a11, a21 a22, a31 a32 a33, ..., an1 an2 ... ann.
...
![A diagram showing a square field with a circular track inside. A small black rectangle representing a train car is on the track at the bottom.](373e2ccc5c19a052a1b8cb6de3eebc94_img.jpg)

A diagram showing a square field with a circular track inside. A small black rectangle representing a train car is on the track at the bottom.
...
1. 如右图所示，共有 13 个格子。对任何一个格子进行一次操作，会使得它自己以及与它上下左右相邻的格子中的数字改变（由 1 变 0，或由 0 变 1）。现在要使得所有的格子中的数字都变为 0，至少需要 次操作。

![A 5x5 grid of numbers with some cells containing 0 or 1, representing a state for problem 1.](d3ca266c298aeb34b019960c6c36f187_img.jpg)

A 5x5 grid of numbers with some cells containing 0 or 1, representing a state for problem 1.

2. 如下图所示，A 到 B 是连通的。假设删除一条细的边的代价是 1，删除一条粗的边的代价是 2，要让 A、B 不连通，最小代价是 （2 分），最小代价的不同方案数是 （3 分）。（只要有一条删除的边不同，就是不同的方案）

![A graph with nodes A and B and several intermediate nodes connected by thick and thin edges, representing a network for problem 2.](f0bae10b54c4f3cf8d0e33f5e2fb7cfa_img.jpg)

A graph with nodes A and B and several intermediate nodes connected by thick and thin edges, representing a network for problem 2.
