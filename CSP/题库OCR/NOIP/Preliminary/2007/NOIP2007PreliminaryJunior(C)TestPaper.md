...
![Logo of the Chinese Computer Society (CCF)](2173c0102e23a5ff29d90d4353fc0339_img.jpg)
...
![Logo of the National Olympiad in Informatics (NOI)](440e59dae4772c0152116a3abd34331a_img.jpg)

Logo of the National Olympiad in Informatics (NOI)
...
1. (求字符串的逆序) 下面的程序的功能是输入若干行字符串, 每输入一行, 就按逆序输出该行, 最后键入 **CTRL+Z** 终止程序 (程序中 EOF 为输入终止的标记, 相应的按键为 **CTRL+Z**, 机内码为 -1)。
...
int reverse(char s[])
{int i,j,t;
  for(i=0,j=strlen(s)-2; i<j; ③ )  /*s[strlen(s)-1]为换行符*/
    {t=s[i]; s[i]=s[j]; s[j]=t;}
  return 0;
}
main()
